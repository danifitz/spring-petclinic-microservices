# New Relic Open Instrumentation Observability Demo
New Relic - Open, Connected, Programmable. This repo demonstrates the ability to collect metrics, logs and traces without the use of an agent.

This is a fork of the Spring PetClinic demo built on Spring Cloud that demonstrates open instrumentation using New Relic. We demonstrate the following:
* Metrics - collected using Micrometer/Spring Actuator and the New Relic Micrometer exporter
* Logs - generated using Logback, each microservice logs to a file which is sent to New Relic using Fluentd
* Traces - Spring Cloud Sleuth/Zipkin traces are generated and sent to New Relic's Trace API

## Usage

1. Clone the repo
2. Build the fluentd docker image `docker build -t fluentd:newrelic .`
3. Build the Spring Petclinic microservices `INSIGHTS_INSERT_KEY=YOUR_KEY NEW_RELIC_METRIC_URI=https://metric-api.newrelic.com/metric/v1 NEW_RELIC_TRACE_URI=https://trace-api.newrelic.com/trace/v1 ./mvnw clean install -PbuildDocker`
4. Optional: Reminder - make sure to the use the EU API endpoints if needed!
4. Run the microservices `docker-compose up`
5. Open [New Relic One](https://one.newrelic.com) and check out the Metrics, Logs and Traces
...
In order to start entire infrastructure using Docker, you have to build images by executing `INSIGHTS_INSERT_KEY=<YOUR_KEY> ./mvnw clean install -PbuildDocker` 
